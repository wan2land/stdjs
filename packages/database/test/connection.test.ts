...
import { connect, Testcase } from './utils'
...
describe('testsuite of connection', () => {
  for (const testcase of testcases) {
    ...
    it(`test first on ${testcase}`, async () => {
      ...
      try {
        await expect(connection.first(selectSqls[testcase])).rejects.toEqual(new RowNotFoundError())
    ...
    it(`test transaction fail on ${testcase}`, async () => {
      ...
      try {
        ...
        await expect(connection.first(selectSqls[testcase])).rejects.toThrow('row not found.')
