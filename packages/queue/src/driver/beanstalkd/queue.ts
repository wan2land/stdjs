...
export class BeanstalkdQueue<TPayload> implements Queue<TPayload> {
  ...
  public constructor(public client: any, public tube = 'default') {
  }
  ...
  public async receive(): Promise<BeanstalkdJob<TPayload> | undefined> {
    await this.connect()
  ...
  public async delete(job: BeanstalkdJob<TPayload>): Promise<void> {
    await this.connect()
    await this.client.delete(job.id)
    job.isDeleted = true // eslint-disable-line require-atomic-updates
  }
}
